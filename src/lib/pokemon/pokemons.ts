...
export const getAggregatedPokemons = async () => {
  ...
  console.info('Done: P.getPokemonsList()');

  const responses = [];

  // ※ promise all だと負荷がかかってタイムアウトするので for await を使う
  for (const result of results) {
    try {
      const pokemon = await P.getPokemonByName(result.name);
      if (pokemon.id > 9999) continue; // 10000番台は除外（フォームとかフォルム系）
            ...
            ? evolutionChain.chain.evolves_to
                .flatMap((ev) =>
                  ev.evolves_to.length > 0
                    ? ev.evolves_to.map((ev2) => getIdFromUrl(ev2.species.url))
                    : [],
                )
                .filter((ev) => ev !== null)
            : null,
